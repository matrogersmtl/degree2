...
class TestDeg2fcFunctions(unittest.TestCase):
    ...
    def test_eisenstein(self):
        prec = 10
        es4, es6, es10, es12 = [
            eisenstein_series_degree2(k, prec) for k in [4, 6, 10, 12]
        ]
        f10 = es4 * es6 - es10
    ...
    def test_hecke_operator(self):
        ...
        self.assertEqual(es4.hecke_operator_acted(2, 5), 45 * es4._down_prec(5))
        f10 = rankin_cohen_pair_sym(2, es4, es6)
        self.assertEqual(f10.hecke_operator_acted(2, 5), -6168 * f10._down_prec(5))
    ...
    def test_basis_of_scalar_valued(self):
        for k in [12, 16, 35, 37, 47]:
            ...
            self.assertTrue(
                matrix([[f[t] for f in M.basis()] for t in tpls])
                .change_ring(QQ)
                .is_invertible()
            )
