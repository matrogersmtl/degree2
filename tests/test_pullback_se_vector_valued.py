...
from degree2.diff_operator_pullback_vector_valued import (
    ...
    _Z_U_ring,
    _diff_z_exp,
    fc_of_pullback_of_diff_eisen,
    _U_ring,
    sqcap_mul,
    D_tilde_nu,
    algebraic_part_of_standard_l,
    _pullback_vector,
    _u3_u4_nonzero,
)
...
class TestPullBackVectorValued(unittest.TestCase):
    ...
    def test_bracket_power_ad_bracket(self):
        for n in range(2, 6):
            for _ in range(1000):
                ...
                for p in range(n + 1):
                    self.assertEqual(
                        ad_bracket(m, p) * bracket_power(m, p),
    ...
    def test_diff_z_exp(self):
        ...
        for _ in range(500):
            ...
            r_pol = sum(
                a * b
                for a, b in zip(random_vector(QQ, 5), (z11**a for a in range(5)))
            )
            pol_exp = r_pol * exp(n * z11)
            r_pol_diff_se = (
                derivative(pol_exp, z11, m) / exp(n * z11)
    ...
    def test_pullback_diff_eisen_scalar_wt12(self):
        x12 = x12_with_prec(5)
        self.assert_pullback_scalar_valued(
            x12, (1, 1, 1), [(2, 1, 3), (2, 0, 2), (2, 2, 3)], 10
        )
    ...
    def test_pullback_diff_eisen_scalar_wt12_diff4(self):
        x12 = x12_with_prec(5)
        self.assert_pullback_scalar_valued(
            x12, (1, 1, 1), [(2, 1, 3), (2, 0, 2), (2, 2, 3)], 8, verbose=True
        )
    ...
    def test_pullback_diff_eisen_scalar_wt35(self):
        ...
        self.assert_pullback_scalar_valued(
            x35, (2, 1, 3), [(2, -1, 4), (3, -1, 4), (3, -2, 4)], 34, verbose=True
        )
    ...
    def assert_pullback_scalar_valued(self, f, t0, ts, l, verbose=False):
        ...
        a = fc_of_pullback_of_diff_eisen(l, f.wt, 0, D, D, 1, 1, verbose=verbose)
        ...
        for t in ts:
            ...
            A = tpl_to_half_int_mat(t)
            self.assertEqual(
                fc_of_pullback_of_diff_eisen(l, f.wt, 0, A, D, 1, 1, verbose=verbose),
                a * f[t],
            )
            ...
            for al in range(n + 1):
                for be in range(n + 1):
                    if al + be <= n:
                        p = n - al - be
                        lhs = sqcap_mul(
                            bracket_power(z2 * t4, al),
                            sqcap_mul(
                                identity_matrix(QQ, binomial(n, p)),
                                bracket_power(z2 * t3, be),
                                n,
                                p,
                                be,
                            )
    ...
    def test_delta_p_q_expand(self):
        ...
        for _ in range(50):
            ...
            for q in range(n + 1):
                p = n - q
                lhs = sum(
                    delta_al_be(t1, t2, t4, z2, q - be, be)
                    * (-1) ** be
                    * binomial(q, be)
                    for be in range(q + 1)
                )
                rhs = sqcap_mul(
                    bracket_power(t1 * z2, q)
                    * bracket_power(t4 - QQ(1) / QQ(4) * t3 * t1 ** (-1) * t2, q),
                    bracket_power(t2, p),
                    n,
                    q,
                    p,
    ...
    def test_diff_pol_value(self):
        for _ in range(50):
            ...
            for k, nu in [(10, 2), (10, 4), (12, 6)]:
                self.assertEqual(
                    scalar_valued_diff_pol_1(k, nu, t1, t4, t2),
    ...
    def test_pullback_lin_comb1(self):
        ...
        self.assert_pullback_lin_comb(M, f, 6, [(1, 1, 2), (2, 1, 2)], verbose=True)
...
def delta_al_be(t1, t2, t4, z2, al, be):
    ...
    t3 = t2.transpose()
    res = sqcap_mul(
        bracket_power(t1 * z2, al + be)
        * sqcap_mul(
            bracket_power(t4, al), bracket_power(t3 * t1 ** (-1) * t2, be), n, al, be
        ),
        bracket_power(t2, p),
        n,
        al + be,
...
def scalar_valued_diff_pol_2(k, nu, A, D, R):
    ...
    a0 = QQ(D_tilde_nu(k, nu, QQ(1), [ZZ(0)] * 4, A=A0, D=D0).constant_coefficient())
    b0 = QQ(scalar_valued_diff_pol_1(k, nu, A0, D0, tpl_to_half_int_mat((0, 0, 0))))
    a = b0 / a0
