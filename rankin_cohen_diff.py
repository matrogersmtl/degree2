...
from degree2.elements import QexpLevel1, QseriesTimesQminushalf, ModFormQexpLevel1

from degree2.elements import SymWtModFmElt as SWMFE

from degree2.basic_operation import common_prec, common_base_ring, _common_base_ring
...
def diff_opetator_4(f1, f2, f3, f4):
    ...
    res = ModFormQexpLevel1(sum(wt_s) + 3, res.fc_dct, prec_res, base_ring=base_ring)
    return res
...
def rankin_cohen_triple_x5(_Q, _f, _prec, _i=2):
    ...
    raise DeprecationWarning(
        "Use '_rankin_cohen_bracket_func'" " with x5__with_prec instead."
    )
...
def _rankin_cohen_bracket_func(Q, rnames=None, unames=None):
    ...
    def monom_mul(tpl, v, flist):
        tpls = group(tpl, 3)
        l = zip(flist, tpls)
        return (v * mul([QQ(2) ** (-t[1]) for _, t in l])) * mul(
            (f._differential_operator_monomial(*t) for f, t in l)
        )
...
def _rankin_cohen_pair_det2_sym_pol(j, k, l):
    ...
    Q2 = (
        (2 * k - 1) * (2 * l - 1) * detRpS
        - (2 * k - 1) * (2 * k + 2 * l - 1) * detS
        - (2 * l - 1) * (2 * k + 2 * l - 1) * detR
    )
    Q = QQ(4) ** (-1) * Q2 * Q + QQ(2) ** (-1) * (
...
def _rankin_cohen_triple_det_sym4_pol(k1, k2, k3):
    ...
    m00 = matrix(
        [[(k1 + 1) * r11, k2, k3], [r11**2, s11, t11], [r11 * r12, s12, t12]]
    )

    m01 = matrix(
        [[k1, (k2 + 1) * s11, k3], [r11, s11**2, t11], [r12, s11 * s12, t12]]
    )

    m10 = matrix(
        [[(k1 + 1) * r12, k2, k3], [r11 * r12, s11, t11], [r12**2, s12, t12]]
    )

    m11 = matrix(
        [[k1, (k2 + 1) * s12, k3], [r11, s11 * s12, t11], [r12, s12**2, t12]]
    )

    m12 = matrix(
        [[(k1 + 1) * r11, k2, k3], [r11**2, s11, t11], [r11 * r22, s22, t22]]
    )

    m13 = matrix(
        [[k1, (k2 + 1) * s11, k3], [r11, s11**2, t11], [r22, s11 * s22, t22]]
    )

    m20 = matrix(
        [[(k1 + 1) * r12, k2, k3], [r11 * r12, s11, t11], [r22 * r12, s22, t22]]
    )

    m21 = matrix(
        [[k1, (k2 + 1) * s12, k3], [r11, s11 * s12, t11], [r22, s22 * s12, t22]]
    )

    m30 = matrix(
        [[(k1 + 1) * r12, k2, k3], [r12**2, s12, t12], [r12 * r22, s22, t22]]
    )

    m31 = matrix(
        [[k1, (k2 + 1) * s12, k3], [r12, s12**2, t12], [r22, s12 * s22, t22]]
    )

    m32 = matrix(
        [[(k1 + 1) * r22, k2, k3], [r11 * r22, s11, t11], [r22**2, s22, t22]]
    )

    m33 = matrix(
        [[k1, (k2 + 1) * s22, k3], [r11, s11 * s22, t11], [r22, s22**2, t22]]
    )

    m40 = matrix(
        [[(k1 + 1) * r22, k2, k3], [r22 * r12, s12, t12], [r22**2, s22, t22]]
    )

    m41 = matrix(
        [[k1, (k2 + 1) * s22, k3], [r12, s22 * s12, t12], [r22, s22**2, t22]]
    )
...
def _rankin_cohen_triple_det_sym8_pol(k1, k2, k3):
    ...
    def _mat_det(l):
        return matrix([[r11, s11, t11], [r12, s12, t12], l + [2 * k3]]).det()
...
def _bracket_vec_val(vecs):
    ...
    R = PolynomialRing(QQ, names=", ".join([_names(s) for s in ["x", "y", "z"]]))
    gens_x = R.gens()[: j + 1]
    ...
    def _pol(gens):
        return sum([a * u ** (j - i) * v**i for i, a in zip(range(j + 1), gens)])

    f_x, f_y, f_z = [_pol(gens) for gens in [gens_x, gens_y, gens_z]]
    A = matrix([[f_x, f_y], [f_y, f_z]])
...
def vector_valued_rankin_cohen(f, vec_val):
    ...
    """
    if not (
        isinstance(f, (QexpLevel1, QseriesTimesQminushalf))
    ...
    diff_tau = (
        f.differentiate_wrt_tau(),
        f.differentiate_wrt_z() * QQ(2) ** (-1),
        f.differentiate_wrt_w(),
    )

    def diff_v(vec_val):
        forms = [i * f for f, i in zip(vec_val.forms[1:], range(1, vec_val.sym_wt + 1))]
    ...
    def diff_u(vec_val):
        forms = [
            i * f for f, i in zip(vec_val.forms, reversed(range(1, vec_val.sym_wt + 1)))
        ]
        return SWGElt(forms, vec_val.prec, vec_val.base_ring)
    ...
    res1 = (vec_val.wt + sym_wt // 2 - 1) * SWGElt(forms1, prec, base_ring=base_ring)

    forms2 = _bracket_vec_val(_cross_prod_diff(diff_d(vec_val)))
    res2 = f.wt * f * SWGElt(forms2, prec, base_ring=base_ring)

    res = SWMFE((res1 - res2).forms, f.wt + vec_val.wt + 1, prec, base_ring=base_ring)
    return res
...
def _cross_prod_diff(vec_vals):
    ...
    def differential_monom(vec_val, a, b, c):
        forms = [f._differential_operator_monomial(a, b, c) for f in vec_val.forms]
        return SWGElt(forms, vec_val.prec, vec_val.base_ring)
    ...
    return [2 * (d1(f2) - d2(f1)), d1(f3) - d3(f1), 2 * (d2(f3) - d3(f2))]
...
def _cross_prod(v1, v2):
    ...
    return (2 * (a * bd - b * ad), a * cd - c * ad, 2 * (b * cd - c * bd))
...
def m_operator(k1, k2, k3):
    ...
    def x_op_val(f):
        r, s, t = f.parent().gens()
        return f.subs({r: bracket_op(rs), s: bracket_op(ss), t: bracket_op(ts)})
    ...
    def m_op_val(f):
        ...
        brxs = [
            bracket_op(a) * x_op_val(f.derivative(b))
            for a, b in zip([ts, ss, rs], [t, s, r])
        ]
        brcks = [
            bracket_op(_cross_prod(a, b)) for a, b in zip([rs, ts, ss], [ss, rs, ts])
        ]
