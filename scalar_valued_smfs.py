...
from degree2.elements import ModFormQexpLevel1, QexpLevel1, ModFormQsrTimesQminushalf
...
class SiegelEisensteinSeries(ModFormQexpLevel1):
    ...
    def _fourier_coefficient(self, content, det_4):
        ...
        if det_4 < 0:
            ...
            return (
                2
                * quadratic_L_function__exact(2 - k, -det_4)
    ...
    def _fc__unramfactor(self, content, det_4):
        ...
        l = [
            (p, valuation(content, p), (valuation(det_4, p) - valuation(fd, p)) / 2)
            for p in pfacs
        ]
        return reduce(
            operator.mul,
    ...
    def _fc__unramfactor_at_p(self, p, ci, fi, chi):
        k = self.wt
        return self._fc__unramfactor_at_p_1(p, ci, fi + 1) - chi(p) * p ** (
            k - 2
        ) * self._fc__unramfactor_at_p_1(p, ci, fi)
...
def load_cached_gens_from_file(prec):
    ...
    if prec <= prec39 or set(prec) <= set(prec39) | set(prec34_m17_51):
        if prec <= PrecisionDeg2(21):
            ...
            gens_dct = {k: {t: gens_dct1[k][t] for t in prec} for k in gens_dct1.keys()}
            max_prec = prec
...
def load_deg2_cached_gens(key, prec, wt, cuspidal=False):
    if key in Deg2global_gens_dict.keys():
        ...
        if f.prec >= prec:
            ...
            if key in ["es4", "es6"]:
                res = SiegelEisensteinSeries(wt, prec=prec, base_ring=ZZ, fc_dct=fc_dct)
            else:
                res = ModFormQexpLevel1(
                    wt, fc_dct, prec, base_ring=ZZ, is_cuspidal=cuspidal
                )
            res._is_gen = key
            return res
    else:
...
def x10_with_prec_inner(prec):
    ...
    res = (
        QQ(43867) * QQ(2**10 * 3**5 * 5**2 * 7 * 53) ** (-1) * (-es10 + es4 * es6)
    )
...
def x5_jacobi_pwsr(prec):
    ...
    q1 = S.gens()[0]
    eta_3 = sum(
        [QQ(-1) ** n * (2 * n + 1) * q1 ** (n * (n + 1) // 2) for n in range(mn1, mx1)]
    ) + bigO(q1 ** (prec + 1))
    theta = sum(
        [
            QQ(-1) ** n * q1 ** (((2 * n + 1) ** 2 - 1) // 8) * t ** (n + 1)
    ...
    for n, r, m in prec:
        if 4 * n * m - r**2 == 0:
            ...
            if 4 * n1 * m1 - r1**2 > 0:
                fc_dct[(n, r, m)] = sum(
                    [
                        d**4 * jacobi_g(n1 * m1 // (d**2), r1 // d)
                        for d in gcd([n1, r1, m1]).divisors()
                    ]
                )
...
class AbstSpaceOfLevel1(HeckeModule):
    ...
    def linearly_indep_tuples(self):
        ...
        dim = self.dimension()
        tpls = sorted(
            bd.group_by_reduced_forms().keys(),
...
class KlingenEisensteinAndCuspForms(AbstSpaceOfLevel1):
    ...
    def load_basis_from(self, filename):
        ...
        basis = [ModFormQexpLevel1._from_dict_to_object(dct) for dct in dicts]
    ...
    def construction(self, f):
        return sum(
            [a * b._construction for a, b in zip(self._to_vector(f), self.basis())]
        )
