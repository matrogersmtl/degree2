...
from sage.all import QQ, save, load, gcd, PolynomialRing, divisors, mod, vector
...
class QexpLevel1(FormalQexp):
    ...
    def __init__(self, fc_dct, prec, base_ring=QQ, is_cuspidal=False):
        ...
        """
        FormalQexp.__init__(
            self, fc_dct, prec, base_ring=base_ring, is_cuspidal=is_cuspidal
        )
    ...
    def _from_dict_to_object(cls, data_dict):
        ...
        return cls(fc_dct, prec, base_ring=base_ring, is_cuspidal=is_cuspidal)
    ...
    def __add__(self, other):
        if is_number(other):
            ...
            return QexpLevel1(fcmap, self.prec, self.base_ring, is_cuspidal=cuspidal)
        ...
        return QexpLevel1(fcmap, prec, base_ring=bsring, is_cuspidal=cuspidal)

    def __mul__(self, other):
        if is_number(other):
            if other == 1:
                return self
            fcmap = _mul_fourier_by_num(
                self.fc_dct, other, self.prec, self._is_cuspidal
            )
            if hasattr(other, "parent"):
                ...
                bs = self.base_ring
            return QexpLevel1(
                fcmap, self.prec, base_ring=bs, is_cuspidal=self._is_cuspidal
            )
            ...
            res = QexpLevel1(fcmap, prec, base_ring=bsring, is_cuspidal=cuspidal)
            return res
    ...
    def _inverse(self):
        ...
        for a in prec_d_keys:
            for t in prec_dict[a]:
                ...
                res_dict[t] = -sum(
                    [
                        res_dict[u] * self[(t[0] - u[0], t[1] - u[1], t[2] - u[2])]
                        for u in l
                    ]
                )
...
class QseriesTimesQminushalf(FormalQexp):
    ...
    def __getitem__(self, t):
        if self._mul_dct == {}:
            self._mul_dct = {
                (n - QQ(1) / QQ(2), r + QQ(1) / QQ(2), m - QQ(1) / QQ(2)): v
...
class ModFormQsrTimesQminushalf(QseriesTimesQminushalf):
    ...
    def __mul__(self, other):
        ...
        if is_number(other):
            ...
            return ModFormQexpLevel1(
                self.wt + other.wt, res.fc_dct, res.prec, base_ring=res.base_ring
            )
        else:
            return res
    ...
    def __add__(self, other):
        ...
        if isinstance(other, ModFormQsrTimesQminushalf) and self.wt == other.wt:
            return ModFormQsrTimesQminushalf(res.f_part, self.wt)
        else:
            return res
    ...
    def __pow__(self, other):
        ...
        if isinstance(res, QexpLevel1):
            return ModFormQexpLevel1(wt, res.fc_dct, res.prec, base_ring=res.base_ring)
        else:
...
class ModFormQexpLevel1(QexpLevel1, HeckeModuleElement):
    ...
    def __add__(self, other):
        ...
        if is_hol_mod_form(other) and self.wt == other.wt:
            ...
            mo = other.fc_dct
            cuspidal = self._is_cuspidal and other._is_cuspidal
            fcmap = _add_fourier(ms, mo, prec, cuspidal=cuspidal, hol=True)
            return ModFormQexpLevel1(
                self.wt,
                fcmap,
                prec,
    ...
    def __mul__(self, other):
        if is_number(other):
            if other == 1:
                return self
            fcmap = _mul_fourier_by_num(
                self.fc_dct, other, self.prec, cuspidal=self._is_cuspidal, hol=True
            )
            if hasattr(other, "parent"):
                bs = _common_base_ring(self.base_ring, other.parent())
            else:
        ...
        if is_hol_mod_form(other):
            ...
            mo = other.fc_dct
            cuspidal = self._is_cuspidal or other._is_cuspidal
            fcmap = _mul_fourier(ms, mo, prec, cuspidal=cuspidal, hol=True)
            return ModFormQexpLevel1(
                self.wt + other.wt,
                fcmap,
                prec,
    ...
    def __pow__(self, other):
        ...
        return ModFormQexpLevel1(self.wt * other, res.fc_dct, res.prec, res.base_ring)
    ...
    def __neg__(self):
        ...
        return ModFormQexpLevel1(self.wt, res.fc_dct, res.prec, res.base_ring)
    ...
    def satisfies_maass_relation_for(self, n, r, m):
        if (n, r, m) == (0, 0, 0):
            return True
        return self[(n, r, m)] == sum(
            [
                d ** (self.wt - 1) * self[(1, r / d, m * n / (d**2))]
    ...
    def _from_dict_to_object(cls, data_dict):
        ...
        prec = PrecisionDeg2._from_dict_to_object(prec)
        f = ModFormQexpLevel1(
            wt, fc_dct, prec, base_ring=base_ring, is_cuspidal=is_cuspidal
        )
    ...
    def _inverse(self):
        ...
        return ModFormQexpLevel1(
            -self.wt, res.fc_dct, res.prec, base_ring=res.base_ring
        )
    ...
    def divide(self, f, prec):
        ...
        if isinstance(f, ModFormQexpLevel1):
            return ModFormQexpLevel1(self.wt - f.wt, res.fc_dct, prec, res.base_ring)
        else:
            return res
...
class SymWtGenElt(object):
    ...
    def __getitem__(self, t):
        if isinstance(t, tuple) and isinstance(t[0], tuple) and is_number(t[1]):
            tpl, i = t
    ...
    def __eq__(self, other):
        if isinstance(other, SymWtGenElt) and self.sym_wt == other.sym_wt:
...
class SymWtModFmElt(SymWtGenElt, HeckeModuleElement):
    ...
    def __repr__(self):
        return (
            "Vector valued modular form of weight "
            + "det^{wt} Sym({j}) with prec = {prec}".format(
                wt=self.wt, j=self.sym_wt, prec=self.prec
            )
        )
    ...
    def _from_dict_to_object(cls, data_dict):
        forms_dct, wt, prec, base_ring = [
            data_dict[ky] for ky in ["forms", "wt", "prec", "base_ring"]
        ]
        prec = PrecisionDeg2._from_dict_to_object(prec)
        forms = [QexpLevel1._from_dict_to_object(d) for d in forms_dct]
    ...
    def __add__(self, other):
        ...
        if isinstance(other, SymWtModFmElt) and self.wt == other.wt:
    ...
    def __mul__(self, other):
        ...
        if isinstance(other, (ModFormQexpLevel1, ModFormQsrTimesQminushalf)):
            return SymWtModFmElt(res.forms, self.wt + other.wt, res.prec, res.base_ring)
        else:
            return res
    ...
    def _down_prec(self, prec):
        ...
        return SymWtModFmElt(forms_res, self.wt, prec, base_ring=self.base_ring)
    ...
    def hecke_operator_acted(self, m, prec=None):
        ...
        fc_dct = {t: self.hecke_operator(m, t) for t in prec}
        dcts = [
            {t: v.vec[i] for t, v in fc_dct.items()} for i in range(self.sym_wt + 1)
        ]
        forms = [QexpLevel1(d, prec, base_ring=self.base_ring) for d in dcts]
        return SymWtModFmElt(forms, self.wt, prec, base_ring=self.base_ring)

    def divide(self, f, prec, parallel=False):
        res = SymWtGenElt.divide(self, f, prec, parallel=parallel)
        if isinstance(f, ModFormQexpLevel1):
            return SymWtModFmElt(
                res.forms, self.wt - f.wt, prec, base_ring=res.base_ring
            )
        else:
            return res
    ...
    def change_ring(self, R=None, hom=None):
        forms = [f.change_ring(R=R, hom=hom) for f in self.forms]
        return SymWtModFmElt(forms, self.wt, self.prec, base_ring=forms[0].base_ring)
...
def divide(f, g, prec):
    ...
    for n, r, m in ts_res:
        ...
        s = sum(
            (
                f.fc_dct[(n1 - a, r1 - b, m1 - c)] * res_dct[(a, b, c)]
                for a, b, c in _spos_def_mats_lt((n1, r1, m1))
                if not (
                    (a == n and b == r and c == m)
                    or f.fc_dct[(n1 - a, r1 - b, m1 - c)] == 0
                )
            )
        )
