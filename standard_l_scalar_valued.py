...
def epsilon_tilde_l_k_degree2(l, k, A1, A2):
    ...
    def func(a, _A1, _A2, R, mat):
        return (
            G_y1_y3.subs({y1: R.det() / ZZ(4), y3: mat.det()}).constant_coefficient()
            * a
        )
...
def algebraic_part_of_standard_l(f, l, space_of_cusp_form=None):
    ...
    if f[(1, 1, 1)] != 0 and f[(1, 0, 1)] != 0:
        t = (1, 1, 1)
        A1 = matrix([[ZZ(1), ZZ(0)], [ZZ(0), ZZ(1)]])
    else:
        t = f._none_zero_tpl()
        A1 = tpl_to_half_int_mat(t)
    msg = "l must be an positive even integer less than or equal to %s" % (f.wt,)
    try:
        l = ZZ(l)
    except TypeError:
    ...
    pull_back_dct = {
        t: epsilon_tilde_l_k_degree2(l + 2, f.wt, A1, tpl_to_half_int_mat(t))
        for t in tpls
    }
...
def first_elt_of_kern_of_vandermonde(chply, lam, beta_vec):
    ...
    if phi_d_lam == 0:
        ...
        num = sum(
            sum(beta_vec[d - 1 - j] * chply[d - j + i] for j in range(i, d)) * lam**i
            for i in range(d)
        )
...
def norm_of_normalized_alg_part_of_standard_l(f, l, space_of_cusp_form=None):
    a = algebraic_part_of_standard_l(f, l, space_of_cusp_form=space_of_cusp_form)
