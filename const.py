...
def _scalar_mod_form_consts():
    return {
        k: [ScalarModFormConst(a) for a in v] for k, v in scalar_mod_form_wts.items()
    }
...
class ConstVectValued(ConstVectBase):
    ...
    def calc_form(self, prec):
        ...
        funcs = {2: self._calc_form2, 3: self._calc_form3, 4: self._calc_form4}
        l = len(self.consts)
    ...
    def _calc_form2(self, prec):
        funcs = {0: rankin_cohen_pair_sym, 2: rankin_cohen_pair_det2_sym}
        func = funcs[self.inc]
        forms = self.forms(prec)
        return func(self.sym_wt, *forms)

    def _calc_form3(self, prec):
        funcs = {1: rankin_cohen_triple_det_sym, 3: rankin_cohen_triple_det3_sym}
        func = funcs[self.inc]
        forms = self.forms(prec)
        return func(self.sym_wt, *forms)
    ...
    def _calc_form4(self, prec):
        if self.inc == 1:
            funcs = {
                "a": rankin_cohen_quadruple_det_sym,
            ...
            funcs = {
                "a": rankin_cohen_quadruple_det3_sym,
    ...
    def _latex4(self):
        ...
        if self.type == "a":
            ...
            lrcp = latex_rankin_cohen(
                self.inc - 1, self.sym_wt, [c._latex_() for c in [f1, f2]]
            )
...
class ConstVectValuedHeckeOp(ConstVectBase):
    ...
    @property
    def _key(self):
        return ("ConstVectValuedHeckeOp", self._const_vec._key, self._m)
...
class ConstDivision(ConstVectBase):
    ...
    def _latex_using_dpd_depth1(self, dpd_dct):
        ...
        tail_terms = [
            "%s %s %s" % ("+" if c > 0 else "", c, n) for c, n in coeffs_names[1:]
        ]
...
class ConstMul(ConstVectBase):
    ...
    def __repr__(self):
        return "ConstMul({const}, {scc})".format(
            const=str(self._const_vec), scc=self._scalar_const
        )
    ...
    def _latex_using_dpd_depth1(self, dpd_dct):
        return "%s %s" % (self._scalar_const._latex_(), dpd_dct[self._const_vec])
...
class CalculatorVectValued(object):
    ...
    def all_dependencies(self):
        ...
        return reduce(
            lambda x, y: x.union(y), (dependencies(c) for c in self._const_vecs)
        )
    ...
    def rdeps(self, const):
        ...
        return {
            c
            for c in self.all_dependencies().union(set(self._const_vecs))
    ...
    def calc_forms_and_save(self, prec, verbose=False, do_fork=False, force=False):
        ...
        def msg(c, prc):
            return "{t}: Computing {c} with prec {prc}".format(
                c=repr(c), t=str(time.ctime()), prc=str(prc)
            )
        ...
        def calc_and_save(c, prc):
            def call_back():
                depds_dct = {
                    dp: dp.load_form(self._data_dir) for dp in c.dependencies_depth1()
                }
    ...
    def forms_dict(self, prec):
        return {
            c: (c.load_form(self._data_dir))._down_prec(prec) for c in self._const_vecs
        }
