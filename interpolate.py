...
def _to_polynomial(f, val1):
    prec = f.prec.value
    R = PolynomialRing(QQ if f.base_ring == ZZ else f.base_ring, names="q1, q2")
    q1, q2 = R.gens()
    ...
    res = sum(
        [
            sum(
                [
                    f.fc_dct.get((n, r, m), 0) * QQ(val1) ** r
                    for r in range(
                        -int(floor(2 * sqrt(n * m))), int(floor(2 * sqrt(n * m))) + 1
                    )
                ]
            )
            * q1**n
...
def det_deg2(mat, autom=True, wt=None, num_of_procs=multiprocessing.cpu_count()):
    """
    ...
    if autom:
        return calc_forms(
            func, forms_flatten, bd, autom=True, wt=wt, num_of_procs=num_of_procs
        )
    else:
        return calc_forms(
            func, forms_flatten, bd, autom=False, num_of_procs=num_of_procs
        )
...
def interpolate_deg2(dct, bd, autom=True, parity=None):
    ...
    def interpolate_pol(x, d):
        ...
        return sum(
            [v * prd_dff.subs({x: k}) ** (-1) * prd // (x - k) for k, v in d.items()]
        )
    ...
    def t_pol_dct(n, m):
        ...
        # put u = t + t^(-1)
            ...
            dct_u = {a + a ** (-1): v[(n, m)] / (a - a ** (-1)) for a, v in dct.items()}
            u_pol = interpolate_pol(u, dct_u)
            return t_ring(
                t ** (2 * bd) * u_pol.subs({u: t + t ** (-1)}) * (t - t ** (-1))
            )
...
def calc_forms(
    func, forms, prec, autom=True, wt=None, num_of_procs=multiprocessing.cpu_count()
):
    """
