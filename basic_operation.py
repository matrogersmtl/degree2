...
class PrecisionDeg2(object):
    ...
    def __init__(self, prec):
        ...
        elif isinstance(prec, (frozenset, set, tuple, list)) and all(
            [_is_triple_of_integers(a) for a in prec]
        ):
            self.__prec = frozenset(prec)
            self.__type = "tuples"
        else:
            raise TypeError(
                "self must be an integer or " + "a collection of tuples of integers."
            )
    ...
    def group_by_reduced_forms(self):
        ...
        res0 = {(0, 0, 0): set(r0)}
        res1 = {
            ls[0]: ls for k, ls in list_group_by(r1, lambda t: gcd([QQ(x) for x in t]))
        }
        res2 = {
            ls[0]: ls
            for k, ls in list_group_by(r2, lambda x: reduced_form_with_sign(x)[0])
        }
        res = {}
...
def _partition_mul_fourier(
    ...
    def weight_fn(x):
        n, r, m = x
        return max(16.0 / 9.0 * (ZZ(n) * ZZ(m)) ** (1.5) - ZZ(n) * ZZ(m) * abs(r), 0)
...
def _mul_fourier(mp1, mp2, prec, cuspidal=False, hol=False):
    ...
    tupls_s = _partition_mul_fourier(
        prec, cuspidal=cuspidal, hol=hol, num_of_procs=current_num_of_procs.num_of_procs
    )
    ...
    def _mul_fourier1(tupls):
        return {
            (n, r, m): sum(
                (
                    mp1[(n0, r0, m0)] * mp2[(n - n0, r - r0, m - m0)]
                    for n0, r0, m0 in _spos_def_mats_lt((n, r, m))
                )
            )
            for (n, r, m) in tupls
        }
