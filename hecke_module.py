...
from degree2.utils import _is_triple_of_integers, is_number, uniq, polynomial_func, pmap
...
class HalfIntegralMatrices2(object):
    ...
    def __add__(self, other):
        return HalfIntegralMatrices2(
            (self._n + other._n, self._r + other._r, self._m + other._m)
        )
...
class HeckeModuleElement(object):
    ...
    def _hecke_tp2_for_eigenform(self, p, tpl, lambda_p=None):
        ...
        return sum([v * fc(i, t) for i, t, v in self._hecke_tp2_sum_alst(p, tpl)])
    ...
    def _hecke_tp2(self, p, tpl):
        ...
        """
        return sum(
            v * self[(p**i * n, p**i * r, p**i * m)]
            for i, (n, r, m), v in self._hecke_tp2_sum_alst(p, tpl)
        )
    ...
    def _hecke_tp2_sum_alst(self, p, tpl):
        ...
        def reprs(i2):
            if i2 == 0:
                ...
                l1 = [[[1, 0], [u, p**i2]] for u in range(p**i2)]
                l2 = [[[p * u, p**i2], [-1, 0]] for u in range(p ** (i2 - 1))]
                return l1 + l2
    ...
    def _hecke_tp2_needed_tuples(self, p, tpl):
            ...
            if i == 0:
                ...
                return reduce(
                    ...
                    [
                        [
                            x[0]
                            for x in self._hecke_tp_psum_alst(
                                p, (p**a * n, p**a * r, p**a * m)
                            )
                        ]
                        for a in range(i)
        ...
        for i, t, _ in self._hecke_tp2_sum_alst(p, tpl):
            res += nd_tpls(i, t)
        return [(n, r, m) for n, r, m in res if (n % p, r % p, m % p) != (0, 0, 0)]
    ...
    def _hecke_op_vector_vld(self, p, i, tpl):
        ...
        def term(al, bt, gm, u):
            if not (
                al + bt + gm == i
                and tpl.is_divisible_by(p**gm)
                and tpl[u].is_divisible_by(p ** (gm + bt))
            ):
                return zero
            else:
...
class HeckeModule(ModularFormModule):
    ...
    def eigenform_with_eigenvalue_t2(self, lm):
        ...
        """
        res = self.eigenvector_with_eigenvalue(lambda f, t: f.hecke_operator(2, t), lm)
...
def reprs_of_double_cosets(p, i):
    ...
    if i == 0:
        ...
        l1 = [[[1, 0], [u, p**i]] for u in range(p**i)]
        l2 = [[[p * u, p**i], [-1, 0]] for u in range(p ** (i - 1))]
        return l1 + l2
...
class SymTensorRepElt(object):
    ...
    def _to_pol(self):
        ...
        m = self.sym_wt
        return sum(
            [a * u1 ** (m - i) * u2**i for a, i in zip(self.vec, range(m + 1))]
        )
    ...
    def group_action(self, mt):
        ...
        dt = (a * d - b * c) ** self.wt
        vec = vector(
            [dt * vec_pl[(self.sym_wt - i, i)] for i in range(self.sym_wt + 1)]
        )
    ...
    def __add__(self, other):
        if other == 0:
            return self
        elif isinstance(other, SymTensorRepElt) and self.wt == other.wt:
    ...
    def __rmul__(self, other):
        if is_number(other):
            ...
            and isinstance(other.parent(), sage.matrix.matrix_space.MatrixSpace)
        ):
