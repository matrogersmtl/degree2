...
from degree2.const import CalculatorVectValued, ConstDivision, ConstMul, ConstVectValued
...
_wt12_consts = [cvv([SMFC([4]), SMFC([4, 4])]), cvv([SMFC([4]), SMFC([6])], inc=2)]
...
def _wt18_mul_basis(prec):
    ...
    for consts, wt in alst:
        res.extend(
            [d[c] * f for c in consts for f in ModularFormsDegree2(wt, prec).basis()]
        )
...
def _wt6_const():
    coeffs = [
        Integer(756104669) / Integer(1684996983384000),
...
def _wt8_const():
    coeffs = [
        -Integer(4730030099) / Integer(2808328305640000),
...
def _wt10_klingen_const():
    ...
    """
    coeffs = [
        -Integer(83651648095008) / Integer(72529140125),
...
def _wt14_consts_mul():
    res = [ConstMul(_wt6_const(), SMFC([4, 4])), ConstMul(_wt8_const(), SMFC([6]))]
...
def test_det_is_divisible_x35_fifth():
    ...
    f187 = ModFormQexpLevel1.load_from(os.path.join(data_dir, "gens_even_det.sobj"))
