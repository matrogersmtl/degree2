...
class RamanujanConj(unittest.TestCase):
    ...
    def assert_ramanujan_conj_eigenform(self, f, complex_prec=300):
        ...
        self.assertTrue(charpoly.is_irreducible(), "charpoly is not irreducible.")
        K = f.base_ring
    ...
    def test_known_eigenforms(self):
        ...
        for c in klingen_consts:
            self.assertEqual(
                forms_dct[c].euler_factor_of_spinor_l(2), _hecke_pol_klingen(c.weight())
            )
        self.assertEqual(
            forms_dct[krs_const].euler_factor_of_spinor_l(2), _hecke_pol_krs_lift()
        )
